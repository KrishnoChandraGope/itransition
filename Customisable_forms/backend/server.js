...
const db = mysql.createConnection({
  ...
  user: 'root', 
  password: '', 
  database: 'itransition', // name of the database
});
db.connect((err) => {
  if (err) {
...
app.get("/api/forms", (req, res) => {
  ...
  console.log("Backend received user_id:", user_id);

  if (user_id === 0) {  //When request comes from (Quiz ManuBar) Unlogged user
    db.query(
      "SELECT forms.*, users.name FROM forms JOIN users ON forms.user_id = users.id",
      (err, results) => {
        if (err) {
  ...
  else {  // When request come from loged in user
    ...
    db.query("SELECT role FROM users WHERE id = ? LIMIT 1", [user_id], (err, roleResults) => {
      ...
      if (userRole === "user") {
        ...
        query = "SELECT forms.*, users.name FROM forms JOIN users ON forms.user_id = users.id";
      } else {
...
// Gretrive Questions from (Selected form view).............................................................
app.get("/api/forms/:formId", (req, res) => {
  db.query("SELECT * FROM questions WHERE form_id = ?", [req.params.formId], (err, results) => {
